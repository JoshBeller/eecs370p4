...
int main(){
    stateType state;
    state.pc = 2;
    state.numMemory = 2; // = max(size of machine code file, largest writeback address + 1)
    state.mem[0] = 0x00810001;
    state.mem[1] = 0x01800000;
    for(int i = 0; i<NUMREGS; i++) state.reg[i] = 0;
    state.reg[1] = state.mem[1];
    printAction(0,1,memoryToCache);
    printAction(0,1,cacheToProcessor);
    printAction(0,1,cacheToNowhere);
    printAction(1,1, memoryToCache);
    printAction(1,1,cacheToProcessor);
    printAction(1,1,cacheToProcessor);
    printState(&state);
    printf("$$$ Main memory words accessed: 2\n");
}